...
public class LsTest {
    ...
    public void testLsCommand() throws IOException, InterruptedException {
        ...
        String command = gcloudPath + " storage ls gs://" + bucketName;
        ...
        if (exitCode != 0) {
            System.err.println("gcloud storage ls command failed with exit code: " + exitCode);
            System.err.println("Error output: " + errorOutput.toString());
            fail("gcloud storage ls command failed: " + errorOutput.toString());
        }

        String outputStr = output.toString();
        System.out.println("gcloud ls output:\n" + outputStr);

        assertTrue(outputStr.contains("gs://" + bucketName + "/testfile.txt"), "Output does not contain expected file.");
    }
}